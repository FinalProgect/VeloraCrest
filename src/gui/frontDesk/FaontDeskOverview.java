...
import model.RoomStatusGUIUpdatorFrantDeskOverView;
...
//        occupancyGraphic();
        ...
        roomStatusUpdatorStator();

    }

    ///Update Room Status worker Starter
    private void roomStatusUpdatorStator() {
        if (!RoomStatusGUIUpdatorFrantDeskOverView.FrantDeskOvervireRoomsUpdatorStarted) {
            RoomStatusGUIUpdatorFrantDeskOverView updateRoomsGUI = new RoomStatusGUIUpdatorFrantDeskOverView(this);
            updateRoomsGUI.start();
            System.out.println("RRRoooMMM SSttaattuussUUppddaattoorrTThhrreeddSSttaatteedd . . . .");
        }
    }
    ///Update Room Status worker Starter
    ...
    public void updateRoomStatus() {
        Component[] rooms = jPanel19.getComponents();

        for (int i = 0; i < rooms.length; i++) {

            JPanel room = (JPanel) rooms[i];

            JLabel roomNumberLabel = (JLabel) room.getComponent(1);

            int ocupiedStatus = roomsMap.get(roomNumberLabel.getText()).getOccupideStatus();

            int roomCleanStatus = roomsMap.get(roomNumberLabel.getText()).getRoomCleanStatus();

            System.out.println("rooom oupied status ===============" + ocupiedStatus);
            System.out.println("rooom Cleaning Status ===============" + roomCleanStatus);

            if (ocupiedStatus == 1) {
                room.setBackground(new Color(255, 0, 0)); // Set background color RED
            } else {

                switch (roomCleanStatus) {
                    case 1:
                        room.setBackground(new Color(60, 179, 113)); // Set background color GREEN
                        break;
                    case 3:
                        room.setBackground(new Color(62, 161, 217)); // Set background color BLUE
                        break;
                    default:
                        room.setBackground(new Color(116, 1, 113)); // Set background color PURPAL
                        break;
                }

            }
            SwingUtilities.updateComponentTreeUI(room);
            System.out.println("RRRRRRRROOOOOOOOOOMMMMMMMMM" + roomNumberLabel.getText());
    ...
    //Start room status cheker
    private void roomStatusCheckerStarter() {
