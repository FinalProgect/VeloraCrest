...
public class CompleteTaskStrategy implements TaskActionStrategyInterface {
    ...
    public void doTask(String taskID, HouseKeepingStaffOverview parent) {
        try {
            ...
            if (rs.next()) {
                ...
                MYsql.execute(String.format("UPDATE `taskshedule` SET `taskStatus_id`  = '3' WHERE `id` = '%s'  ", taskID));
                MYsql.execute(String.format("UPDATE `cleanroom` SET `Cleanstatus_id` = '1' WHERE `rooms_id` = '%s' ", roomID));
                parent.loadCards();
